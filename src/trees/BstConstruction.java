...
public class BstConstruction {
    ...
    public static List<Integer> branchSums(BST root) {
        List<Integer> brancSumsList = new ArrayList<>();
        calculateBranchSum(root,0,brancSumsList);
        return brancSumsList;
    }

    private static void calculateBranchSum(BST root, int sum, List<Integer> list) {
        if(root == null)
            return;
        sum = sum + root.value;
        ...
        calculateBranchSum(root.left,sum,list);
        calculateBranchSum(root.right,sum,list);
    }
...
//                .insert(4500);
//        test.inorder();
        List<Integer> resultList = branchSums(test);
